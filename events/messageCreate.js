...
module.exports = async (client, message) => {
  ...
  if (cmd && !message.guild && cmd.conf.guildOnly)
    return message.channel.send({ embeds: [{
      ...
      title: "❌ This command is unavailable via private message. Please run this command in a guild.",
      footer: {
  ...
  // Some commands may only be used in DMs. This check prevents those commands from running
  // and return a friendly error message.
  if (cmd && message.guild && cmd.conf.dmsOnly)
    return message.channel.send({ embeds: [{
      color: embedSettings.wrongcolor,
      title: `❌ Please use the \`${cmd.help.name}\` command in private DM's.`,
      footer: {
        text: embedSettings.footertext,
        icon_url: embedSettings.footericon
      }
